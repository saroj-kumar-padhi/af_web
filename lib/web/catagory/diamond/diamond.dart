...
class DiamondPageWeb extends StatelessWidget {
  const DiamondPageWeb({super.key});
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: ListView.builder(
            itemCount: imoDiamond.length, // Number of items in your list
            itemBuilder: (BuildContext context, int index) {
              return Padding(
                padding:
                    const EdgeInsets.only(bottom: 20, left: 150, right: 150),
                child: InkWell(
                  onTap: () => showDialogPhone(),
                  child: CustomWallet6Web(
                    image: 'assets/imo.jpeg',
                    text: imoDiamond[index],
                    //  text2: 'MTN to MTN',
                    text1: imoDiamond2[index],
                  ),
                ),
              );
            }));
